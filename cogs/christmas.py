...
class Christmas:
    ...
    async def lotto_winners(self, ctx):
        entries = deque([ctx.guild.get_member(x) for x, in await self.bot.db.fetch("SELECT user_id FROM lotto_entries")])
        winners = [ctx.guild.get_member(x) for x, in await self.bot.db.fetch("SELECT user_id FROM lotto_winners")]
        ...
        rewards = {
            ...
            '<:purpleball:390204323960848386> Purple ball': .10,
            '🍏 Green apple ball': .25
        }
        ...
        n = math.ceil(len(entries) * .10) + 1
        output = "<:purpleball:390204323960848386> Purple ball\n"
        output += ', '.join([x.mention for x in entries[1:n]]) + '\n<:space:390204819979370508>'
        await ctx.send(output)
